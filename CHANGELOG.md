...
- An edge between a subgraph and a node inside it now meets the node on the side the line arrives from. It used to aim at the subgraph's centre, which is on the far side of the member, so the line ran straight through the node and parked its arrowhead underneath — invisible, since nodes paint over edges.
- Edges inside a subgraph can be clicked and double-clicked again. The box was hit-tested before the edges it contains, and a box covers its whole interior, so every edge drawn inside a subgraph was unreachable: clicks selected the subgraph and double-clicks opened its title editor. Nodes still take precedence over edges, and empty space inside the box still selects the subgraph.
